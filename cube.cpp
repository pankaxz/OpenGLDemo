//
// Created by pankaj on 01-10-2024.
//
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
void read_file(const char* path, char* buffer, size_t max_size) {
...
void compile_shader_from_file(const char* path, GLuint shader) {
  ...
  if (!status) {
    char* info_buffer = static_cast<char *>(malloc(0x1000));
...
void initialize(struct context* context) {
  ...
  //set up the vertices relative to the window coordinates
  float vertices[] = {
    // Front face
  ...
  //define the relationship between the vertices to form the triangles
